...
export const configFileName = "scaffolding.config.json";
...
async function readConfig(configPath: string): Promise<Partial<ScaffoldingConfig>> {
  try {
    return JSON.parse(await fs.readFile(configPath, "utf8")) as Partial<ScaffoldingConfig>;
  } catch (error) {
    if (error instanceof SyntaxError) {
      throw new CliError(`${configFileName} must be valid JSON.`);
    }
    throw error;
  }
}
...
function validateConfig(config: Partial<ScaffoldingConfig>): asserts config is ScaffoldingConfig {
  assertCli(config && typeof config === "object" && !Array.isArray(config), `${configFileName} must contain a config object.`);
